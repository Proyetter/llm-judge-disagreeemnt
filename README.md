...
1. How does cross-order disagreement divide into within-order stochasticity and systematic cross-order differences?
2. How do observed instability patterns vary across judges?
3. Does confidence from the first evaluation identify items that show instability across the six-call record?
...
| Split | Response generator | Response pairs |
|---|---|---:|
...
| Developer | Model identifier | Eligible Claude items | Eligible GPT items |
|---|---|---:|---:|
| Anthropic | `anthropic/claude-3-haiku` | 269 | 350 |
| Anthropic | `anthropic/claude-sonnet-5` | 260 | 343 |
| DeepSeek | `deepseek/deepseek-chat` | 270 | 350 |
| Google | `google/gemini-2.5-flash` | 269 | 348 |
| Meta | `meta-llama/llama-3.1-8b-instruct` | 269 | 345 |
| Meta | `meta-llama/llama-3.3-70b-instruct` | 268 | 350 |
| OpenAI | `openai/gpt-4o-mini` | 270 | 350 |
...
| Category | Within-order instability | Majority reversal |
|---|---|---|
| Stable | No | No |
| Noise-only | Yes | No |
| Position-only | No | Yes |
| Tangled | Yes | Yes |
...
| Resource | Location or specification |
|---|---|
| Manuscript version used for this release | TODO |
| Saved judgment records and field definitions | TODO |
| JudgeBench source and dataset version | TODO |
| Exact prompt and per-model configuration | TODO |
| Tested Python version and dependency file | TODO |
| Verified installation command | TODO |
| Reference result files | TODO |

### Analysis commands

Replace each TODO with the tested command, including required arguments and input paths.

| Step | Command | Expected output |
|---|---|---|
| Audit judgment records | TODO | Record validity and eligible counts by judge and split |
| Reproduce decomposition results | TODO | Mean disagreement components, NoiseShare, and intervals |
| Reproduce category results | TODO | Category proportions, conditional rates, and differences |
| Reproduce confidence results | TODO | AUROCs, intervals, and outcome-specific sample sizes |

Compare the regenerated outputs with the manuscript, including the eligible counts above. Record the code version and random seeds used for the comparison.
